...
function quoteFor(baseAmount, price) {
    return (baseAmount * price) / 10n ** 18n;
}
export function tryMatch(newOrderHash) {
    ...
    if (!incomingAuth || incomingAuth.status !== "active")
        return null;
    const opposite = [...orders.values()]
        ...
        // Crossing logic
        .filter((o) => {
        if (incoming.order.side === SIDE_BUY_BASE)
            return incoming.order.price >= o.order.price;
